...
###########################
# MARK: - Inlier Oxford Timing on 1k features
###########################
...
for i in range(len(DetectorsLegend)):
    for j in range(len(DescriptorsLegend)):
        ...
        average_inlier_count = np.nanmean(np.concatenate((Rate_graf[:, :, i, j,  9], Rate_wall[:, :, i, j,  9], Rate_trees[:, :, i, j,  9], Rate_bikes[:, :, i, j,  9], Rate_bark[:, :, i, j,  9], Rate_boat[:, :, i, j,  9], Rate_leuven[:, :, i, j,  9], Rate_ubc[:, :, i, j,  9]), axis=0))
        mean_total_time      = np.nanmean(np.concatenate((Exec_time_graf  [:, :, i, j, 0] + Exec_time_graf  [:, :, i, j, 1] + Exec_time_graf  [:, :, i, j, 2],
                                                          Exec_time_wall  [:, :, i, j, 0] + Exec_time_wall  [:, :, i, j, 1] + Exec_time_wall  [:, :, i, j, 2],
                                                          Exec_time_trees [:, :, i, j, 0] + Exec_time_trees [:, :, i, j, 1] + Exec_time_trees [:, :, i, j, 2],
                                                          Exec_time_bikes [:, :, i, j, 0] + Exec_time_bikes [:, :, i, j, 1] + Exec_time_bikes [:, :, i, j, 2],
                                                          Exec_time_bark  [:, :, i, j, 0] + Exec_time_bark  [:, :, i, j, 1] + Exec_time_bark  [:, :, i, j, 2],
                                                          Exec_time_boat  [:, :, i, j, 0] + Exec_time_boat  [:, :, i, j, 1] + Exec_time_boat  [:, :, i, j, 2],
                                                          Exec_time_leuven[:, :, i, j, 0] + Exec_time_leuven[:, :, i, j, 1] + Exec_time_leuven[:, :, i, j, 2],
                                                          Exec_time_ubc   [:, :, i, j, 0] + Exec_time_ubc   [:, :, i, j, 1] + Exec_time_ubc   [:, :, i, j, 2]), axis=0))                                                        
        result3 = (mean_total_time / average_total_count)  * 1000 if average_total_count  > 0 else np.nan
        result4 = (mean_total_time / average_inlier_count) * 1000 if average_inlier_count > 0 else np.nan
        if not np.isnan(result3):
...
fig551 = go.Figure()
fig551.update_layout(margin=dict(l=20, r=20, t=25, b=25))
x = [f'Img{i}' for i in range(153, 188)]
fig551.update_layout(  xaxis = dict(tickmode = 'array', tickvals = x))
fig551.update_yaxes(title_text="Correctly matched point rates %")
color_index = 0
for i in range(len(DetectorsLegend)):
    for j in range(len(DescriptorsLegend)):
        for c3 in range(len(Norm)):
            ...
            color = colors[color_index]
            style = line_styles[j % len(line_styles)]
            legend_groupfig551 = f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Norm[c3]}'
            if not (np.isnan(Rate_drone[:, c3, i, j, 11]).any()):
                fig551trace_Drone  = go.Scatter(x=x, y=Rate_dr,  mode='lines', line=dict(color=color, dash=style), name=legend_groupfig551, legendgroup=legend_groupfig551, showlegend=True)
                fig551.add_trace(fig551trace_Drone)
            color_index += 1
fig551.write_html("./html/droneData.html")
###########################
# MARK: - Drone Timing on Average 1k features
###########################
Exec_time_drone   = np.load('./arrays/Exec_time_drone.npy')
fig56 = make_subplots(rows=3, cols=2, subplot_titles=['Average 1k Detect time', 'Average 1k Describe time', 'Average 1k Total time (Detect + Descript + Match + RANSAC)', 'Average 1k Inlier time (Detect + Descript + Match + RANSAC)'], shared_xaxes=False, shared_yaxes=False, specs=[[{}, {}],[{"colspan": 2}, None],[{"colspan": 2}, None]],horizontal_spacing=0.05, vertical_spacing=0.1)
fig56.update_layout(margin=dict(l=20, r=20, t=25, b=25))
fig56.update_yaxes(title_text="seconds", row=1, col=1)
fig56.update_yaxes(title_text="seconds", row=1, col=2)
fig56.update_yaxes(title_text="seconds", row=2, col=1)
fig56.update_yaxes(title_text="seconds", row=3, col=1)
# detector time
for i in range(len(DetectorsLegend)):
    mean_keypoint2_number = np.nanmean(Rate_drone[:, :, i, :, 6])
    mean_detector_time    = np.nanmean(Exec_time_drone[:, :, i, :, 0])    
    result = (mean_detector_time / mean_keypoint2_number) * 1000
...
# descriptor time
for j in range(len(DescriptorsLegend)):
    mean_descriptor2_number = np.nanmean(Rate_drone[:, :, :, j, 8])
    mean_descriptor_time    = np.nanmean(Exec_time_drone[:, :, :, j, 1])
    result2 = (mean_descriptor_time / mean_descriptor2_number) * 1000
...
for i in range(len(DetectorsLegend)):
    for j in range(len(DescriptorsLegend)):
        average_total_count  = np.nanmean(Rate_drone[:, :, i, j, 10])
        average_inlier_count = np.nanmean(Rate_drone[:, :, i, j,  9])
        mean_total_time      = np.nanmean(Exec_time_drone[:, :, i, j, 0] + Exec_time_drone[:, :, i, j, 1] + Exec_time_drone[:, :, i, j, 2])
        result3 = (mean_total_time / average_total_count)  * 1000 if average_total_count  > 0 else np.nan
        result4 = (mean_total_time / average_inlier_count) * 1000 if average_inlier_count > 0 else np.nan
        if not np.isnan(result3):
            trace_match_drone_result3 = go.Bar(x=[DetectorsLegend[i] + '-' + DescriptorsLegend[j]], y=[result3], name=DetectorsLegend[i] + '-' + DescriptorsLegend[j], showlegend=True, text=[f'{result3:.4f}'], textposition='auto')
            fig56.add_trace(trace_match_drone_result3, row=2, col=1)
        if not np.isnan(result4):
            trace_match_drone_result4 = go.Bar(x=[DetectorsLegend[i] + '-' + DescriptorsLegend[j]], y=[result4], name=DetectorsLegend[i] + '-' + DescriptorsLegend[j], showlegend=True, text=[f'{result4:.4f}'], textposition='auto')
            fig56.add_trace(trace_match_drone_result4, row=3, col=1)
fig56.write_html("./html/drone_timing_Average1k.html")