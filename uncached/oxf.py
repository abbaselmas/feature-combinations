        ...
        for c3 in range(len(matching)):
            for i in range(len(Detectors)):
                for j in range(len(Descriptors)):
                    ...
                    if k == 3 and drawing:
                        keypointImage1 = cv2.drawKeypoints(img[0],          keypoints1,  None, color=( 25, 255, 255), flags=0)
                        ImageGT        = cv2.drawKeypoints(keypointImage1,  keypoints11, None, color=(240, 240,  70), flags=0)
                        keypointImage2 = cv2.drawKeypoints(img[k],          keypoints2,  None, color=( 25, 255, 255), flags=0)
                        Image2         = cv2.drawKeypoints(keypointImage2,  keypoints22, None, color=(230,  50, 240), flags=0)
