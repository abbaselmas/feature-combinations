...
Descriptors    = list([sift, akaze, orb, brisk, kaze, daisy, freak, brief, lucid, latch, vgg675, vgg625, vgg500, vgg075, beblid675, beblid625, beblid500, beblid100, teblid675, teblid625, teblid500, teblid100, boost675, boost625, boost500, boost150, boost075]) 
...
Rate = np.zeros((nbre_img,   len(matching), len(Detectors), len(Descriptors), 14))
Exec_time = np.zeros((nbre_img,   len(matching), len(Detectors), len(Descriptors), 3))
        ...
        for i in range(len(Detectors)):
            for j in range(len(Descriptors)):
                try:
                    ...
                    Exec_time[k, c3, i, j, 0] = time.time() - start_time
                    ...
                    start_time = time.time()
                    Rate[k, c3, i, j, 13], good_matches, matches = evaluate_scenario_intensity(matcher, keypoints1, keypoints2, descriptors1, descriptors2, matching[c3])
...
np.save(f"{maindir}/arrays/Rate_intensity.npy",      Rate)
np.save(f"{maindir}/arrays/Exec_time_intensity.npy", Exec_time)
headers = [
...
Rate = np.zeros((len(scale), len(matching), len(Detectors), len(Descriptors), 14))
Exec_time = np.zeros((len(scale), len(matching), len(Detectors), len(Descriptors), 3))
        ...
        for i in range(len(Detectors)):
            for j in range(len(Descriptors)):
                try:
                    ...
                    Exec_time[k, c3, i, j, 1] = time.time() - start_time
                    start_time = time.time()
                    Rate[k, c3, i, j, 13], good_matches, matches = evaluate_scenario_scale(matcher, keypoints1, keypoints2, descriptors1, descriptors2, matching[c3], scale[k])
                    Exec_time[k, c3, i, j, 2] = time.time() - start_time
                    Rate[k, c3, i, j, 2] = len(keypoints1)
                    Rate[k, c3, i, j, 3] = len(keypoints11)
                    Rate[k, c3, i, j, 4] = len(keypoints2)
...
np.save(f"{maindir}/arrays/Rate_scale.npy", Rate)
np.save(f"{maindir}/arrays/Exec_time.npy", Exec_time)
headers = [
        "K", "Detector", "Keypoint1", "Keypoint11", "Keypoint2", "Keypoint22",
        "Descriptor", "Descriptor1", "Descriptor2", "Matching", "Matcher",
        "Inliers", "Total Matches", "Match Rate",
...
Rate = np.zeros((len(rot),   len(matching), len(Detectors), len(Descriptors), 14))
Exec_time = np.zeros((len(rot),   len(matching), len(Detectors), len(Descriptors), 3))
        ...
        for i in range(len(Detectors)):
            for j in range(len(Descriptors)):
                try:
                    ...
                    Exec_time[k, c3, i, j, 1] = time.time() - start_time
                    start_time = time.time()
                    Rate[k, c3, i, j, 13], good_matches, matches = evaluate_scenario_rotation(matcher, keypoints1, keypoints2, descriptors1, descriptors2, matching[c3], rot[k], rot_matrix)
                    Exec_time[k, c3, i, j, 2] = time.time() - start_time
                    Rate[k, c3, i, j, 2] = len(keypoints1)
                    Rate[k, c3, i, j, 3] = len(keypoints11)
                    Rate[k, c3, i, j, 4] = len(keypoints2)
...
np.save(f"{maindir}/arrays/Rate_rot.npy", Rate)
np.save(f"{maindir}/arrays/Exec_time.npy", Exec_time)
headers = [
        "K", "Detector", "Keypoint1", "Keypoint11", "Keypoint2", "Keypoint22",
        "Descriptor", "Descriptor1", "Descriptor2", "Matching", "Matcher",
